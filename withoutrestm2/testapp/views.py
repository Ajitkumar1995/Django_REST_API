...
class StudentCRUDCBV(SerializeMixin,HttpResponseMixin,View):
    ...
    def get(self,request,*args,**kwargs):
        ...
        if id is not None:
            ...
            if std is None:
                return self.render_to_http_response(json.dumps({'msg':'No Matches resource found for the given id'}),status=400)
    ...
    def post(self,request,*args,**kargs):
        ...
        if form.errors:
            ...
            return self.render_to_http_response(json.dumps(json_data,status=400))

    def put(self,request,*args,**kwargs):
        data=request.body
        valid_json=is_json(data)
        if not valid_json:
            return self.render_to_http_respone(json.dumps({'msg':'please send valid js on only'}),status=400)
    ...
    def delete(self,request,*args,**kwargs):
        ...
        id=data.get('id',None)
        if id is None:
            return self.render_to_http_response(json.dumps({'msg':'To perform deelete id is mandatory you should provide'}),status=400)
        obj=self.get_object_by_id(id)
        if obj is None:
            json_data=json.dumps({'msg':'No matched record found, Not possible to per from delete operations'})
            return self.render_to_http_response(json_data,ststus=404)
        status,deleted_item=obj.delete()
        if status==1:
            json_data=json.dumps({'msg':'Resource deleted successfully'})
            return self.render_to_http_response(json_data)
        json_data=json.dumps({'msg':'unable to delete.. plz try gagain'})
